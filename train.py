...
def train(train_samples, valid_samples, test_samples, index, word2vec_preweight, vocabulary_dim, process_file, jpg_file,
          save_model_file,test_label_file, para_dict,temp_max):
    # print ('Traing begin')
    ...
    train_statement_data = torch.from_numpy(train_statement_data).float().cuda()
    ...
    train_meta_data = torch.from_numpy(train_meta_data).long().cuda()
    ...
    train_target = torch.from_numpy(train_target).int().cuda()

    # valid data
    valid_statement_data = [x[1] for x in valid_samples]
    valid_statement_data = np.array(valid_statement_data)
    valid_statement_data = torch.from_numpy(valid_statement_data).float().cuda()
    ...
    valid_meta_data = torch.from_numpy(valid_meta_data).long().cuda()
    ...
    test_statement_data = torch.from_numpy(test_statement_data).float().cuda()
    ...
    test_meta_data = torch.from_numpy(test_meta_data).long().cuda()
    ...
    for epoch in range(para_dict['EPOCH']):
        ...
        for step, (batch_statement, batch_statement_len, batch_meta, batch_history, batch_y) in enumerate(train_loader):
            ...
            batch_y = Variable(batch_y).long().cuda()
            ...
            if step % display_interval == 0:
                ...
                if test_acc_max<test_acc:
                    ...
                    temp_max = test_acc
                    with open("C:/Users/lenovo/PycharmProjects/me/Encoder_4/test_label.txt", "r") as f:
                        data = f.read()
                    with open("C:/Users/lenovo/PycharmProjects/me/Encoder_4/test_label_max.txt", "w",
                              encoding='utf-8') as f:
                        f.seek(0)
                        f.truncate()
                        f.write(data)
                ...
                process_file.write(
                    '......==>Epoch:' + str(epoch) + ' ' + 'valid_Loss=' + str(valid_loss) + ' ' + 'test_Acc=' + str(
                        test_acc) + '\r\n')
    #print(test_acc_max, best_test_acc)
    ...
    plt.xlim(0, para_dict['EPOCH']* (len(train_samples)//para_dict['batch_size']//display_interval+1))
    #plt.ylim(min(test_acc_list), max(test_acc_list))
    plt.ylim(0.45,0.50)
    plt.ylabel('Acc')
    plt.xlabel('Iter')
    ...
    return best_test_acc,test_acc_max,temp_max
